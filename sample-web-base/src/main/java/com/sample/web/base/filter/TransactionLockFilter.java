...
public class TransactionLockFilter extends OncePerRequestFilter {
    ...
    public TransactionLockFilter(TransactionLockService transactionLockService) { // コンストラクタでBeanを受け取る
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (sessionCookie != null && isUpdateRequest(request)) {
            ...
            if(transactionLockService.exists(key)) {
                ...
                response.sendRedirect(request.getContextPath() + ERROR_URL + HttpStatus.CONFLICT.value());
                return;
            }
